...
starttab[n]
		bits[		n-1] = 0				bits[n,PLST] = 1
endtab[n] =
		bits[		n-1] = 1				bits[n,PLST] = 0
...
maskPPWbits(x, w, startmask, endmask, nlw)
		...
		afbGlyphBlt to put down glyphs <= PPW bits wide.
...
getleftbits(psrc, w, dst)
		...
		in dst.  this is used by the afbGlyphBlt code for glyphs
		<=PPW bits wide.
 ...
 * name		afb32	afb64	explanation
 * ----		------	------	-----------
 * PGSZ		32			64			pixel group size (in bits; same as PPW for afb)
...
/* warning: PixelType definition duplicated in afb.h */
#ifndef PixelType
